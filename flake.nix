            ...
            packages = [
              ...
              (pkgs.writeShellApplication {
                ...
                text = ''
                  ...
                  if [ -f ~/.ssh/desktop ]; then
                    IDENTITY=~/.ssh/desktop
                  elif [ -f ~/.ssh/laptop ]; then
                    IDENTITY=~/.ssh/laptop
                  else
                  ...
                  cd secrets
                  agenix -i $IDENTITY -d key.properties.age > ../android/key.properties
                  agenix -i $IDENTITY -d upload-keystore.jks.age > ../android/upload-keystore.jks
                  cd ..

                  echo "Building..."
                  flutter build appbundle

                  rm android/key.properties android/upload-keystore.jks
